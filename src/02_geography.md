<div class="section" style="text-align:center; margin-top:12px;">
  ...
  <div class="page-subhead">Which boroughs and precincts experience the highest volume of 911 calls?</div>
</div>

<style>
@import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@500;700&family=IBM+Plex+Serif:ital,wght@0,400;0,500;1,400&display=swap');
:root {
  --bg-dark: #2f3437;
  --bg-light: #f2ecdf;
  --fg-strong: #181a1a;
  ...
  --accent-blue: #4a90a4;
  --card-radius: 16px;
  --card-shadow: 0 16px 36px rgba(0,0,0,0.08);
}
body {
...
.squiggle {
  ...
  width: 120px;
  height: 10px;
  background: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="120" height="10" viewBox="0 0 120 10" fill="none"><path d="M0 5c5-5 10 5 15 0s10 5 15 0 10 5 15 0 10 5 15 0 10 5 15 0 10 5 15 0 10 5 15 0" stroke="%233f4644" stroke-width="2" fill="none"/></svg>');
}
.card {
  background: #fff;
...
.observablehq pre, .observablehq code {
...
/* Hide language tag on code blocks, keep copy icon */
.observablehq--language {
...
<div class="section">
  <div class="section-title">Citywide load vs. service speed</div>
  <span class="squiggle"></span>
<div class="obs">
...
```js echo
{
  // Full code reference (matches the rendered chart above)
const scatterData = callCategory === "All Categories"
  ? boroughProfiles
  : boroughProfiles.filter(d => d.category === callCategory);
const selectedBorough = precinctBorough === "Citywide" ? null : precinctBorough;
...
legendSizes.forEach((val, i) => {
  ...
  // (Reference block only; rendering happens in the chart cell above)
}
```
...
<div class="section">
  <div class="section-title">Call intensity across boroughs</div>
  <span class="squiggle"></span>
  <div class="card chart-block">
...
```js echo
{
  // Reference code (rendered above; display omitted to avoid duplicate output)
  const mapWidth = 420;
  const mapHeight = 520;
  const projection = d3.geoMercator().fitSize([mapWidth, mapHeight], boroughBoundaries);
  const path = d3.geoPath(projection);
...
<div class="section">
  <div class="section-title">Where is the mix shifting?</div>
  <span class="squiggle"></span>
<div class="obs">
...
<div class="section">
  <div class="section-title">Drill into precinct workloads</div>
  <span class="squiggle"></span>
  <div class="card chart-block">
...
<div class="section">
  <div class="section-title">When do borough workloads spike?</div>
  <span class="squiggle"></span>
  <div class="card chart-block">
